...
<tbody>
    ...
    <tr>
    <tr>
    <td>sent date</td><td><?php echo $senttime; ?> </td>
    </tr>
        <td>delivered date</td><td><?php echo $delivered_time; ?> </td>
    </tr>
    
    <td>payment type</td><td><?php echo "BY ".$Payinfo; ?> </td>
    </tr>
    </tbody>
</table><br/>
...
<?php } ?>
 ...
 <h2>Actions</h2>
 <form method="post">
 <input type="submit" name="send_delivery" value="Delivery Completed">
...
if(isset($_POST['send_delivery']))
{
    ...
    if($query1)
    {
        $query = "UPDATE `bs_record` SET `Ostatus` = '1' WHERE `bs_record`.`id` = $order_id;";
        $query1=mysqli_query($link,$query);
        $query2 = "UPDATE `bs_record` SET `sent_time` WHERE `bs_record`.`id` = $order_id;";
        $query3=mysqli_query($link,$query);
        header('location:sentdeliveries.php');
    }
